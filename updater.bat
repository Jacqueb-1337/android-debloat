...
curl --silent https://raw.githubusercontent.com/Jacqueb-1337/android-debloat/main/list.txt -L --ssl-no-revoke | for /f "tokens=3" %%a in ('find "List version: "') do @echo %%a>latestver.txt
...
if NOT exist list.txt goto doupdateyn
type list.txt | for /f "tokens=3" %%a in ('find "List version: "') do @echo %%a>thisver.txt
...
curl https://raw.githubusercontent.com/Jacqueb-1337/android-debloat/main/list.txt -L --ssl-no-revoke -o list2.txt
:next
